...
function setModelViewId (modelViewConstructor) {
    ...
    // construct a limited object out of the unique parts of model view
    // in order to calculate a unique id
    const modelViewId = _.omit(modelView, ['each', 'post', 'pre', 'constructor', 'module'])
    if (modelView.each) {
        if (modelView.each.meta) {
            modelViewId.each = modelView.each.meta.functionId
        }
        else {
            modelViewId.each = stableId(modelView.each.function)
        }
    }
    if (modelView.post) {
        if (modelView.post.meta) {
            modelViewId.post = modelView.post.meta.functionId
        }
        else {
            modelViewId.post = stableId(modelView.post.function)
        }
    }
    if (modelView.pre) {
        if (modelView.each.meta) {
            modelViewId.pre = modelView.pre.meta.functionId
        }
        else {
            modelViewId.pre = stableId(modelView.pre.function)
        }
    }
    // set id on model view
    modelViewConstructor.modelView.modelViewId = stableId(modelViewId)
}
