...
/* public methods */
ImmutableCoreModelView.getModelView = getModelView
ImmutableCoreModelView.hasModelView = hasModelView
ImmutableCoreModelView.looksLikeConstructor = looksLikeConstructor
ImmutableCoreModelView.looksLikeInstance = looksLikeInstance
...
// list of properties function must have to look like constructor
const looksLikeConstructorProperties = {
    // properties
    name: 'string',
    type: 'string',
    // methods
...
// list of properties object must have to look like instance
const looksLikeInstanceProperties = {
    // properties
    args: 'object',
    name: 'string',
    type: 'string',
 ...
 *
 * check if function looks like model view constructor based on properties
 *
 * @returns {boolean}
 */
function looksLikeConstructor (obj) {
    // constructor should be function
    if (typeof obj !== 'function') {
        return false
    }
    // constructor should have modelView object
    if (typeof obj.modelView !== 'object') {
        return false
    }
    // flag will be set false if any properties do not match
    var looksLike = true
    // check each property
    _.each(looksLikeConstructorProperties, (type, name) => {
        // check if property matches
        if (looksLike && typeof obj.modelView[name] !== type) {
            looksLike = false
        }
    })
    // return result
 ...
 *
 * check if object looks like model view instance based on properties
 *
 * @returns {boolean}
 */
function looksLikeInstance (obj) {
    ...
    var looksLike = true
    // check each property
    _.each(looksLikeInstanceProperties, (type, name) => {
        // check if property matches
